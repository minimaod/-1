...
async def run_agent_task(task_id: str, prompt: str, engine: AgentEngine):
    print(f"\n[任务 {task_id} 启动] -> {prompt}\n")
    async for event_type, data in engine.chat_stream(prompt):
        if event_type == "text":
            sys.stdout.write(f"[{task_id}:{data}]")
            sys.stdout.flush()
    print(f"\n[任务 {task_id} 结束]\n")
