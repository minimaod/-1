...
4. 上下文与协议双保护：滑动窗口裁剪必须同时守住 System Pinning（系统提示词永不被挤出）与 Tool Pair Atomicity（tool_calls 与 tool 回执成对保留或成对丢弃），否则长对话下会在 LLM 侧偶发 400 Bad Request。其中 Tool Pair Atomicity 无条件生效；System Pinning 的触发前提是历史首位确实是 system 消息，而 AgentEngine 只在「显式传入 system_prompt 且当前历史为空」时才插入（agent/core.py:42）。agent/session.py 的两个构造点现已一律传入 settings.DEFAULT_SYSTEM_PROMPT，故默认链路上角色认知被钉在 history[0]，这道保护默认生效。唯一的例外是本次改动之前就已落盘的旧会话：冷恢复时历史非空，该守卫会跳过注入，这类会话不会被补上 system 消息（需清空会话或迁移历史才能获得角色认知）。
📂 项目结构 (Project Tree)
mini_agent/
